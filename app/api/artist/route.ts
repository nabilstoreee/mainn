...
import { safeGetArtist, safeSearch, safeSearchSongs, safeSearchVideos } from '@/lib/ytmusic';
...
export async function GET(request: Request) {
  ...
  try {
    let artist = (await safeGetArtist(id)) as any;
    
    // If safeGetArtist succeeded, ensure topVideos are populated and fix carousels
    if (artist) {
    ...
    try {
      const searchRes = await safeSearch(id);
