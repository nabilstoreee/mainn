...
import { 
  INITIAL_SPEED_DIAL, 
  INITIAL_LISTEN_AGAIN, 
  INITIAL_POPULAR_PICKS, 
  INITIAL_COMMUNITY_PLAYLISTS, 
  INITIAL_POPULAR_ARTISTS, 
  INITIAL_CATEGORIES 
...
export default function Home() {
  const [speedDialTracks, setSpeedDialTracks] = useState<Track[]>(INITIAL_SPEED_DIAL);
  const [quickPicksTracks, setQuickPicksTracks] = useState<Track[]>(INITIAL_POPULAR_PICKS);
  const [communityPlaylists, setCommunityPlaylists] = useState<any[]>(INITIAL_COMMUNITY_PLAYLISTS);
  const [artists, setArtists] = useState<any[]>(INITIAL_POPULAR_ARTISTS);
  const [categories, setCategories] = useState<{ key: string; title: string; type: 'song' | 'mixed'; items: any[] }[]>(INITIAL_CATEGORIES);
  ...
  // Metrolist "Dengarkan Lagi" (User history or fallback curated favorites)
  ...
  // Main Home Data Fetching for live updates
    ...
    try {
      ...
      for (let i = 0; i < queries.length; i += 3) {
        ...
        chunkResults.forEach((res) => {
          ...
          const { key, title, data } = res;
          if (key === 'speedDial') {
            setSpeedDialTracks(data.slice(0, 45));
          } else if (key === 'quickPicks') {
            setQuickPicksTracks(data.slice(0, 20));
          } else if (key === 'community') {
            setCommunityPlaylists(data.slice(0, 10));
          } else if (key === 'artists') {
            ...
            setCategories((prev) => {
              ...
              const newItem = {
                key,
                title,
                type: key.startsWith('similar') ? ('mixed' as const) : ('song' as const),
    ...
    <main className="min-h-screen pt-4 pb-32 overflow-x-hidden">
      ...
      >
        <div className="min-w-0 flex-1 pr-3">
          ...
          <h1 suppressHydrationWarning className="text-2xl sm:text-3xl font-black text-white leading-tight mt-0.5 tracking-tight">
            {greeting.title}
          </h1>
      ...
      {/* Mood Pills (Category Filter Chips) */}
      ...
      >
        ...
        {activeMoodId !== 'all' ? (
          ...
          <>
            {/* Speed Dial Grid (Pilihan Cepat Untukmu: 3 rows per column) */}
            ...
            {/* "Dengarkan Lagi" (Listen Again / Favorites) */}
