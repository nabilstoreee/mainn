...
export interface CommunityPlaylistData {
  playlistId: string;
  name: string;
...
interface CommunityPlaylistCardProps {
  playlistId?: string;
  playlistData?: CommunityPlaylistData;
}

function CommunityPlaylistCardComponent({ playlistId: propId, playlistData: propData }: CommunityPlaylistCardProps) {
  const targetId = propData?.playlistId || propId || '';
  
  const [playlist, setPlaylist] = useState<CommunityPlaylistData | null>(() => {
    if (propData && propData.videos && propData.videos.length > 0) {
      return propData;
    }
    return null;
  });
  ...
  const [loading, setLoading] = useState<boolean>(() => {
    return !(propData && propData.videos && propData.videos.length > 0);
  });
  ...
  useEffect(() => {
    ...
    const fetchPlaylist = async () => {
      try {
        const res = await fetch(`/api/ytplaylist?id=${encodeURIComponent(targetId)}`, {
        ...
        setPlaylist({
          playlistId: data.playlistId || targetId,
  ...
  const handleAdd = async (e: React.MouseEvent) => {
    ...
    if (!added) {
      await db.addPlaylist({
        ...
        name: playlist.name,
        img: playlist.thumbnails?.[playlist.thumbnails.length - 1]?.url || playlist.videos?.[0]?.thumbnails?.[0]?.url || '',
  ...
  return (
    ...
    >
      <div className="flex gap-4 mb-6">
        <div className="w-24 h-24 rounded-2xl overflow-hidden relative shrink-0 shadow-lg bg-black/20">
          {playlist.videos && playlist.videos.length >= 4 ? (
            ...
            <SmoothImage 
              src={playlist.thumbnails?.[playlist.thumbnails.length - 1]?.url || playlist.videos[0]?.thumbnails?.[0]?.url || '/placeholder.png'} 
              alt={playlist.name} 
              fill 
              sizes="96px" 
              className="object-cover" 
      ...
      <div className="flex items-center gap-3 mt-auto">
        <button 
          onClick={handlePlay}
          className="w-12 h-12 liquid-glass-green rounded-full flex items-center justify-center hover:scale-105 transition-all shadow-lg cursor-pointer"
        ...
        <button 
          onClick={handleRadio}
          className="w-12 h-12 rounded-full liquid-glass-icon flex items-center justify-center hover:scale-105 transition-all text-white cursor-pointer"
        ...
        >
          {added ? <Check className="w-4 h-4 text-[#81B29A]" /> : <PlusSquare className="w-4 h-4 text-white" />}
        </button>
      </div>
    </div>
  );
